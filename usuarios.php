<?php 
	...
	else if ((strlen($arrDados["idUsuario"]) > 0) && ($arrDados["acao"] === "D"))
	{
		...
		if(mysql_query($strSQL))
		{ 
			$strMsg = "O usuário foi excluido com sucesso ";
		}
		else
		{
			$strMsg = "Erro na query ".mysql_error()." O administrador foi avisado. ";
    ...
    else
	{
		...
		$arrDados["DsSenha"] = mysql_real_escape_string($arrDados["DsSenha"]);	
		$strSQL = "INSERT INTO 
							tele_entregas.teUsuario(NmUsuario, DsEmail, FgStatus, DsSenha) 
					VALUES 
							('".$arrDados["NmUsuario"]."', '".$arrDados["DsEmail"]."', '".$arrDados["FgStatus"]."', '".codificaSenha($arrDados["DsSenha"])."')";
		if(mysql_query($strSQL))
		{ 
